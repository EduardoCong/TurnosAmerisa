  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (codigo == 0) {
        ...
        print('Error en el inicio de sesión: $mensaje y $mensaje2');
      }
    } else {
...
Future<void> loginClients(BuildContext context, String usuario, String password) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      final String mensaje2 = responseData['password_cliente'];

      if (codigo == 0) {
        print('Inicio de sesión exitoso: $mensaje y $mensaje2');

        SharedPreferences prefs = await SharedPreferences.getInstance();
        await prefs.setBool('isLoggedIn', true);
        await prefs.setString('usuario_cliente', usuario);
        await prefs.setString('password_cliente', password);
